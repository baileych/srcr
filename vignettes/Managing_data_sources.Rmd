...
or `src_whatever` statement typically contains credentials used to authenticate
...
`src_name`
  : either the name of a driver function compatible with the
    ...
    included).  For historical reasons, the name of an old-style
    [dplyr](https://cran.r-project.org/package=dplyr) function used to construct
    a data source, typically a database connection, such as `src_postgres` or
    `src_mysql` may be used as well..
