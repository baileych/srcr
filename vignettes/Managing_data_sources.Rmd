...
The \pkg{srcr} package provides an alternative in this area that aims to be easy enough to use for straightforward cases, and flexible enough to adapt to complex projects and environments.  Its workhorse is the `srcr()` function.  As its documentation explains, `srcr()` is an adapter that lets you create a data source of a type known to [DBI](https://cran.r-project.org/web/packages/DBI/) or old-style [dplyr](https://cran.r-project.org/web/packages/dplyr/) connection functions,
...
`src_name`
  : either the name of a driver function compatible with the [DBI](https://cran.r-project.org/web/packages/DBI/) specification, such as `SQLite` or `PostgreSQL` (N.B. the initial `R` in the package name is not included), or the name of an old-style [dplyr](https://cran.r-project.org/web/packages/dplyr/) function used to construct a data source, typically a database connection, such as `src_postgres` or `src_mysql`.
